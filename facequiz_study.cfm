...
<cfquery datasource = "callmeasurement" name = "pull_admin">
	select top 25 leuseradminid, adminname, website_avatar, team, linkedin_page, start_date
	from leuseradmin
	where isactive = 'yes'
...
//			timer_start();
	...
	function updateActiveFaceStudy() {
		$('#study_name').html($('.active-face .face-study-name').html());
		$('#study_department').html($('.active-face .face-study-team').html());
		$('#study_start_date').html($('.active-face .face-study-start-date').html());
		
		/*update the linked in link. If not defined, hide.*/
		$('#study_linkedIn').attr('href',$('.active-face .face-study-linkedIn').html());
		if ($('.active-face .face-study-linkedIn').html() == '')	{
			$('#study_linkedIn').hide();
	...
	function scoreSubmit() {
		...
		$('.congrats p').text('scoreSubmit was called!');
		/*
			...
			<div class="container">
				<div class = "left-section">
					<!----<div class="left-section-inner">
						<h1 class = "Name_game">Face Quiz - Study!</h1>
						<div class = "timer">
				...
				<div class = "right-section">
					<div class="right-section-inner">
						<div class="faces-form">
							<cfif isDefined("URL.study")>
								<h1 id="study_name">Developer Dan</h1>
								<h3 id="study_department">Hacker Farm</h3>
								<!----<h4 id="study_start_date">2009-01-01</h4>--->
								
								<a href="##" id="study_linkedIn"><img src="linkedin-logo-hover.png" style="height:20px; width:20px;"></a>
							<cfelse>
								<h3>Who is this?</h3>
									...
									<cfset namearray = ArrayNew(1)>
										<cfloop query = "pull_admin">
											<div class="face-circle inactive-face" data-index = "#pull_admin.currentRow#">
												...
												<cfif isDefined("URL.study")>
													<div style="display:none;" class="face-study-name">#pull_admin.adminname#</div>
													<div style="display:none;" class="face-study-team">#pull_admin.team#</div>
													<div style="display:none;" class="face-study-start-date">#dateformat(pull_admin.start_date)#</div>
													<div style="display:none;" class="face-study-linkedIn">#pull_admin.linkedin_page#</div>
												</cfif>
